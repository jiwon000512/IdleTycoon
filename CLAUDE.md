...
- 설명·비교·보고는 아티팩트로 한다. 그 작업이 끝나 다음 작업으로 넘어가면 사용자가 남기라고 한 것이 아닌 한 **그 작업에서 만든 아티팩트(설계 NN 포함)를 묻지 않고 삭제**하고 문서의 링크도 지운다(2026-09-26). 남는 것은 기획서 아티팩트뿐이며, 결정은 기획서 「확정 결정」표와 진행상황 글에 남긴다.
- 커밋은 묻지 않는다(2026-09-26). **완료 직후에는 커밋하지 않는다** — 사용자가 직접 플레이해 보고 피드백을 줄 시간이며, 그 피드백 수정은 같은 커밋에 들어간다. 사용자가 **다음 작업을 언급하는 순간**(또는 "커밋"이라고 하면 즉시) 검증이 끝난 앞 작업을 에이전트가 알아서 커밋 + `origin main` 푸시한다. 검증이 안 끝난 것은 커밋하지 않는다. 메시지는 한국어 한 줄 요약 + 필요 시 본문.
