...
기획서: `기획/동물원-타이쿤-기획서.md` (현재 v0.4). 기획과 개발을 병행하며 기획서는 결정이 바뀔 때마다 버전을 올린다.
...
└─ ProjectTycoon/        Unity 프로젝트 (Unity 6000.3.24f1, URP 2D)
   └─ Assets/
      ...
      │  ├─ Data/    ScriptableObject 정의 (AnimalData, GradeData, ZooLevelData)
      ...
      ├─ Resources/Data/     SO 테이블 에셋 (Resources.Load로 읽음)
      ├─ Prefabs/, Prefabs/UI/
      ├─ Sprites/Animals/, Sprites/UI/, Fonts/, Audio/
...
| 데이터 | ScriptableObject 테이블 3개(Animal/Grade/ZooLevel). 가차·홍보 비용은 수식 상수 |
...
- 기획서의 숫자를 코드에 하드코딩할 때는 SO 필드나 상수 하나로 모으고, 기획서 표 번호(예: 6.3)를 주석에 남긴다.
