# -*- coding: utf-8 -*-
# 웜뱃 걷기 프레임(칸 편집, AI 없음, 2026-09-18 확정). 캔버스 좌우 1칸 여백(88px 폭, 가운데 정렬 그대로).
...
SPEC = {
    'front': {'low': 35, 'lean': 9, 'foot': 38, 'left': (8, 16), 'right': (25, 33), 'ear': 4, 'ears': [(5, 13), (28, 36)]},
    'back': {'low': 36, 'lean': 9, 'foot': 42, 'left': (8, 16), 'right': (25, 33), 'ear': 5, 'ears': [(4, 12), (29, 37)]},
}
...
for name, s in SPEC.items():
    base = pad(np.asarray(Image.open(D + f'wombat_{name}.png').convert('RGBA'))[::2, ::2])
    down = remove_row(base, s['low'])
    contact = ears_at(down, base, s, 0)
    frames = [contact, lift(base, s, 'left'), contact, lift(base, s, 'right')]
    for i, f in enumerate(frames):
        Image.fromarray(np.repeat(np.repeat(f, 2, 0), 2, 1)).save(D + f'wombat_{name}_walk_{i}.png')
